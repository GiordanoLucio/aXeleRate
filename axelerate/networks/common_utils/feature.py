...
def create_feature_extractor(architecture, input_size, weights = None):
    ...
    elif architecture == 'Pruned Tiny Yolo':
        feature_extractor = PrunedTinyYolo(input_size, weights)
    else:
...
class SelfExtractor (BaseFeatureExtractor):
    ...
    def __init__(self, input_size, weights):
        ...
        x = Conv2D(24, (3,3), strides=(1,1), padding='same', name='conv_2', use_bias=False)(x)
        x = BatchNormalization(name='norm_2')(x)
        x = LeakyReLU(alpha=0.1)(x)
        x = MaxPooling2D(pool_size=(2, 2))(x)

        x = Conv2D(48, (3,3), strides=(1,1), padding='same', name='conv_3', use_bias=False)(x)
        ...
        x = Conv2D(312, (3,3), strides=(1,1), padding='same', name='conv_7', use_bias=False)(x)
...
class ReducedTinyYoloFeature(BaseFeatureExtractor):
    ...
    def __init__(self, input_size, weights):
        ...
        x = Conv2D(256, (3,3), strides=(1,1), padding='same', name='conv_4', use_bias=False)(x)
        x = BatchNormalization(name='norm_4')(x)
        x = LeakyReLU(alpha=0.1)(x)
        x = MaxPooling2D(pool_size=(2, 2), strides=(1,1), padding='same')(x)

        x = Conv2D(312, (3,3), strides=(1,1), padding='same', name='conv_5', use_bias=False)(x)
        x = BatchNormalization(name='norm_5')(x)
        x = LeakyReLU(alpha=0.1)(x)
...
class PrunedTinyYolo(BaseFeatureExtractor):
    """docstring for ClassName"""
    def __init__(self, input_size, weights):
        input_image = Input(shape=(input_size[0], input_size[1], 3))
